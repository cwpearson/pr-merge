...
for d in $HOME/cfs_m3918/pearson/*; do
  ...
  echo $d;
  kind=$(basename $d)
  script="$GENERATED_DIR/cusparse-spmv-$kind.sh"
...
#SBATCH -J cusparse-spmv-$kind
#SBATCH -e cusparse-spmv-$kind.e%j
#SBATCH -o cusparse-spmv-$kind.o%j
...
--benchmark_filter="MatrixMarket.*/i64/f64/u64/non-conj/.*default" \
