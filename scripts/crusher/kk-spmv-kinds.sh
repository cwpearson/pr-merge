...
for d in $HOME/csc465/proj-shared/cpearson/suitesparse/*; do
  ...
  echo $d;
  kind=$(basename $d)
  script="$GENERATED_DIR/kk-spmv-$kind.sh"
...
#SBATCH -o kk-spmv-$kind.o%j
#SBATCH -e kk-spmv-$kind.e%j
#SBATCH -J kk-spmv-$kind
...
--benchmark_filter="MatrixMarket.*/i32/f64/i32/non-conj/.*native" \
