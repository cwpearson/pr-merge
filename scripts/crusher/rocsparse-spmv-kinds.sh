...
for d in $HOME/csc465/proj-shared/cpearson/suitesparse/*; do
  ...
  echo $d;
  kind=$(basename $d)
  script="$GENERATED_DIR/rocsparse-spmv-$kind.sh"
...
#SBATCH -o rocsparse-spmv-$kind.o%j
#SBATCH -e rocsparse-spmv-$kind.e%j
#SBATCH -J rocsparse-spmv-$kind
...
--benchmark_filter="MatrixMarket.*/i32/f64/i32/non-conj/.*default" \
